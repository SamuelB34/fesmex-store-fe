...
export default function Home() {
  ...
  const { items, page, totalPages, isLoading, error, fetchArticles } = useArticles();
  ...
  const handleAddToCart = async (article: ArticleListItem) => {
    ...
    await addItem({ article_id: article._id, quantity: 1, unit_price: article.price });
    setAddingId(null);
  };
            ...
            {items.map((article) => (
              <div key={article._id} className={styles.card}>
                ...
                <div className={styles.cardFooter}>
                  ...
                  {article.stock && (
                    <span className={article.stock.count ? styles.stock : styles.stockOut}>
                      {article.stock.count > 0 ? `${article.stock.count} in stock` : "Out of stock"}
                    </span>
                  )}
                </div>
                <button
