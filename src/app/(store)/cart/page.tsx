...
function CartContent() {
  ...
  if (isLoading) {
    return <p style={{ textAlign: "center", padding: "2rem", color: "#666" }}>Loading cart...</p>;
  }
  ...
  return (
    <div>
      ...
      <h1 style={{ fontSize: "1.75rem", fontWeight: 600, marginBottom: "1.5rem", color: "#333" }}>Shopping Cart</h1>
      ...
      {items.length === 0 ? (
        <div style={{ textAlign: "center", padding: "3rem 0", color: "#666" }}>
          ...
          <Link href="/" style={{ color: "#007bff", fontWeight: 500 }}>Browse articles</Link>
        </div>
      ) : (
        <>
          <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "1.5rem" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #ddd", textAlign: "left" }}>
                ...
                <th style={{ padding: "0.75rem", color: "#333" }}>Unit Price</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.article_id} style={{ borderBottom: "1px solid #eee" }}>
                  ...
                  <td style={{ padding: "0.75rem", color: "#333" }}>${item.unit_price.toFixed(2)}</td>
                  <td style={{ padding: "0.75rem", color: "#333" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                      <button
                        onClick={() => handleQuantityChange(item, item.quantity - 1)}
                        disabled={updatingId === item.article_id}
                        style={{ width: "32px", height: "32px", border: "1px solid #ddd", borderRadius: "4px", cursor: "pointer", background: "white" }}
                      >
                        -
                      </button>
                  ...
                  <td style={{ padding: "0.75rem", fontWeight: 600 }}>${Number(item.total ?? 0).toFixed(2)}</td>
                  <td style={{ padding: "0.75rem" }}>
                    <button
                      onClick={() => handleRemove(item.article_id)}
                      disabled={updatingId === item.article_id}
                      style={{ padding: "0.375rem 0.75rem", backgroundColor: "#dc3545", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "0.8125rem" }}
                    >
                      Remove
                    </button>
                  </td>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: "2rem", padding: "1rem 0", borderTop: "2px solid #ddd" }}>
            <span style={{ fontSize: "1.25rem", fontWeight: 600 }}>
              Subtotal: ${subtotal.toFixed(2)}
            </span>
            <button
              onClick={() => router.push("/checkout")}
              style={{ padding: "0.75rem 2rem", backgroundColor: "#28a745", color: "white", border: "none", borderRadius: "4px", fontSize: "1rem", fontWeight: 500, cursor: "pointer" }}
            >
              Checkout
            </button>
          </div>
