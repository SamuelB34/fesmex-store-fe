...
const forgotPassword = (payload: ForgotPasswordPayload) =>
  unwrap<{ sent: boolean }>(api.post("/password/forgot", payload, { skipAuth: true } as AuthRequestConfig));

const resetPassword = (payload: ResetPasswordPayload) =>
  unwrap<{ reset: boolean }>(api.post("/password/reset", payload, { skipAuth: true } as AuthRequestConfig));

const verifyEmail = (payload: EmailTokenPayload) =>
  unwrap<{ verified: boolean }>(api.post("/email/verify", payload, { skipAuth: true } as AuthRequestConfig));

const resendEmail = (payload: { email: string }) =>
  unwrap<{ sent: boolean }>(api.post("/email/resend", payload, { skipAuth: true } as AuthRequestConfig));
