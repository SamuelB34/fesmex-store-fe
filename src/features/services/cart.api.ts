...
const addItem = (payload: CartItemPayload) =>
	unwrap<{ cart: Cart }>(api.post('/cart/items', payload))

const updateItem = (payload: CartUpdatePayload) =>
	unwrap<{ cart: Cart }>(api.patch('/cart/items', payload))

const deleteItem = (articleId: string) =>
	unwrap<{ cart: Cart }>(api.delete(`/cart/items/${articleId}`))
