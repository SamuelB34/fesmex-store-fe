...
const toStringOrNull = (value: string | number | null | undefined): string | null => {
...
const normalizeShippingAddress = (address?: RawShippingAddress | null): ShippingAddress => {
  const addr = address ?? {};
  return {
...
const normalizeOrder = (input: RawOrder): Order => {
  ...
  if (!input._id) {
    throw new ApiRequestError("Malformed response: missing required order fields");
  }
...
const unwrapOrThrow = async <T>(promise: Promise<AxiosResponse<ApiResponse<T>>>): Promise<T> => {
...
const createOrder = async (payload: CreateOrderPayload) => {
  const data = await unwrapOrThrow<{ order: RawOrder }>(api.post("/orders", payload));
  return { order: normalizeOrder(data.order) };
};

const listOrders = async (query?: ListOrdersQuery) => {
  const data = await unwrapOrThrow<{ items: RawOrder[]; page: number; limit: number; total: number; totalPages: number }>(
...
const getOrderById = async (id: string) => {
  const data = await unwrapOrThrow<{ order: RawOrder }>(api.get(`/orders/${id}`));
  return { order: normalizeOrder(data.order) };
};

const updateShipping = async (id: string, payload: ShippingFeePayload) => {
  const data = await unwrapOrThrow<{ order: RawOrder }>(api.patch(`/orders/${id}/shipping`, payload));
  return { order: normalizeOrder(data.order) };
};
