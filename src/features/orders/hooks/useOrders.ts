...
export function useOrdersList() {
	...
	const fetchOrders = useCallback(async (query?: ListOrdersQuery) => {
		...
		try {
			...
			const message =
				err instanceof ApiRequestError ? err.message : 'Failed to load orders'
...
export function useOrderDetail() {
	...
	const fetchOrder = useCallback(async (id: string) => {
		...
		try {
			...
			const message =
				err instanceof ApiRequestError ? err.message : 'Failed to load order'
			setState({ order: null, isLoading: false, error: message })
		}
	}, [])

	const updateShippingFee = useCallback(
		async (id: string, payload: ShippingFeePayload) => {
